...
int existe(char* vcedula) {
	...
	while (fgets(linea, sizeof(linea), file)) {
		...
		if (token) {
			strncpy_s(cedula, sizeof(cedula), token, _TRUNCATE);
...
Clientes SolicitarCliente() {
	...
	while (valida == 0) {
		...
		scanf_s("%9s", cliente.Cedula, (unsigned)_countof(cliente.Cedula));

		if (!existe(cliente.Cedula)) {
			printf("Ingrese el nombre: ");
			scanf_s("%49[^\n]", cliente.Nombre, (unsigned)_countof(cliente.Nombre));

			valida = 1;
			while (valida != 0) {
				printf("Ingrese el teléfono: ");
				scanf_s("%14s", cliente.Telefono, (unsigned)_countof(cliente.Telefono));
				printf("\n");
